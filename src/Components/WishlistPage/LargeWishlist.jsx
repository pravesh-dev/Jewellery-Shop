...
function LargeWishlist() {
    ...
    // Get the product at index 3 (if exists)
    const product = wishlist.length > 3 ? items.find(item => item.id === wishlist[3]) : null;

    if (!product) {
        return <div className='min-h-[60vh] text-center text-lg mt-6'>No product found in wishlist</div>;
    }
            ...
            <div className='grid grid-cols-12'>
                ...
                <div className='col-span-4 space-y-6 lg:space-y-10'>
                    ...
                    <div className='h-[6.8rem] grid content-start pt-5 grid-cols-3 gap-2 lg:pt-10'>
                        <span className='font-lora text-center'><span className='text-[0.7rem] py-[0.13rem] px-3 rounded-[0.23rem] text-accent bg-[#ff0000] lg:text-[1rem]'>
                        ...
                        <h3 className='font-Cormorant text-primary text-center text-[0.72rem] italic lg:text-[1.12rem]'>In stock</h3>
                    </div>
                </div>
