    ...
    <div className="w-72 flex flex-col gap-3 md:w-80 lg:w-96 xl:w-[26.4rem] xl:gap-4">
      ...
      <div className="w-full flex gap-2 xl:justify-between">
        {product.image.slice(0, 3).map((img, index) => (
          <div key={index} className="w-1/4 h-20 overflow-hidden rounded-lg lg:h-20 xl:h-[5.8rem] xl:rounded-xl xl:w-[5.9rem]" onClick={()=> changeImage(img)}>
            <img
              src={img}
              className="w-full h-full object-cover"
        ...
        {product.image.length === 4 && (
          <div className="w-1/4 h-20 overflow-hidden rounded-lg lg:h-20 xl:h-[5.8rem] xl:rounded-xl xl:w-[5.9rem]" onClick={()=> changeImage(product.image[3])}>
            <img
              src={product.image[3]}
              className="w-full h-full object-cover"
              alt="other product related image"
