...
// Animation settings
const ringAnimationUp = {
  y: [-20, 20], // Initially at -20, then moves to 20
  transition: {
    duration: 2,
    repeat: Infinity,
    repeatType: "reverse",
...
function ShopBy() {
  return (
    <section className="w-full font-bellefair px-2 py-5 md:px-16 md:py-10 lg:py-28 xl:flex flex-col items-center relative overflow-x-hidden xl:pb-16">
      ...
      {/* Left Animated Ring - Initially at -20, then moves to 20 */}
      <motion.img 
        src={ring} 
        className="hidden md:block absolute left-0 top-28 w-32 lg:w-44 lg:top-64 lg:-left-10 xl:-left-20 xl:top-36 xl:w-[13.3rem]" 
        alt="ring image" 
        animate={ringAnimationUp} 
      />

      {/* Right Animated Ring - Initially at 20, then moves to -20 */}
      <motion.img 
        src={ring} 
        className="w-20 absolute right-0 top-0 scale-x-[-1] sm:w-32 lg:w-44 lg:top-16 xl:top-24 xl:w-[13.3rem] xl:-right-10" 
        alt="ring image" 
        animate={ringAnimationDown} 
      />
